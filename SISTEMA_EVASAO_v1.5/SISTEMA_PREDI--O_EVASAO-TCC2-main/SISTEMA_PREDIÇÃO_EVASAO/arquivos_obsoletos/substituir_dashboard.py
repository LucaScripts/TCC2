...
# Script para substituir a função dashboard_powerbi_tab

# Nova função
nova_funcao = '''def dashboard_powerbi_tab():
...
# Encontrar início e fim da função dashboard_powerbi_tab
inicio = None
fim = None

for i, linha in enumerate(linhas):
    if 'def dashboard_powerbi_tab():' in linha:
        inicio = i
    elif inicio is not None and linha.startswith('def ') and 'dashboard_powerbi_tab' not in linha:
        fim = i
        break

if inicio is not None and fim is not None:
    # Criar novo conteúdo
    novo_conteudo = linhas[:inicio] + [nova_funcao + '\n\n'] + linhas[fim:]
    
    # Salvar arquivo atualizado
    with open('interface_web_limpa.py', 'w', encoding='utf-8') as f:
        f.writelines(novo_conteudo)
    
    print("✅ Função dashboard_powerbi_tab substituída com sucesso!")
    print(f"📍 Substituída nas linhas {inicio+1} a {fim}")
    print("🎨 Dashboard agora será exibido embarcado na interface!")
else:
